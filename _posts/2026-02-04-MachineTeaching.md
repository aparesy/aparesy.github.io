...
With a group of friends and a researcher from INRIA, we are currently working on a year-long project to build tools in order to enable intelligent teaching. The official title for the project is "An intelligent assistant for teaching programming" because we decided to work in particular in the scope of programming (that's where the largest datasets are) but really our idea is to make something that can be generalized to all fields.
